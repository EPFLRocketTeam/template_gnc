...
public:
  ...
  void updateGuidance()
  {
    ...
    else if (rocket_fsm.state_machine.compare("Coast") != 0)
    {
      ...
      {
        double predicted_apogee = rocket.check_apogee(current_state.propeller_mass + rocket.dry_mass,
                                                      current_state.pose.position.z, current_state.twist.linear.z);
...
int main(int argc, char** argv)
{
  ...
  ros::Timer control_thread =
      nh.createTimer(ros::Duration(0.200), [&](const ros::TimerEvent&) { guidanceNode.updateGuidance(); });
